...
#else
#define DIM    768       // Square size of solver domani
#endif
...
#if defined(SAGE_APP)
	glReadPixels(0, 0, wWidth, wHeight, GL_RGB, GL_UNSIGNED_BYTE, rgbBuffer);
	sageInf.swapBuffer();
...
int initGL(int argc, char **argv)
{
    ...
    glutIdleFunc(idle);
         ...
         scfg.winX = 0;
         scfg.winY = 0;
         scfg.winWidth = 2*wWidth;
         scfg.winHeight = 2*wHeight;
         scfg.imageMap = cudaMap;
	...
	rgbBuffer = (GLubyte *)sageInf.getBuffer();
         ...
         fprintf(stderr, "sail initialized\n");
#endif
