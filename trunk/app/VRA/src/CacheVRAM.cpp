...
CacheVRAM::~CacheVRAM() {
  ...
  CacheVRAMNode* current = NULL;
  CacheVRAMNode* next = NULL;
...
void CacheVRAM::Assert(const char* message) {

#ifdef LOG

  // Print message and time to log
  char buffer[1024];
  sprintf(buffer, "CacheVRAM: %s", message);
  Log::Assert(buffer);
...
bool CacheVRAM::IsLoaded(OctreeNode* node) {
...
void CacheVRAM::Load(OctreeNode* node) {
  ...
  // Allocate new node
  CacheVRAMNode* tmp = new CacheVRAMNode;
  if (tmp == NULL) {
    fprintf(stderr, "CacheVRAM: Memory allocation error.\n");
    return;
  }
...
void CacheVRAM::MakeCurrent(OctreeNode* node) {
...
void CacheVRAM::MoveToFront(OctreeNode* node) {
...
void CacheVRAM::Print() {

  // Message
  Assert("Printing CacheVRAM begin.");
  ...
  // Message
  Assert("Printing CacheVRAM end.");

}

/*--------------------------------------------------------------------------*/

void CacheVRAM::RemoveLast() {

  // Message
  Assert("Removing last node begin.");

  // Current and next node pointers used to iterate through cache
  CacheVRAMNode* previous = NULL;
  CacheVRAMNode* current = _front;
  ...
  current -> GetOctreeNode() -> UnloadTexture();
