...
#undef HandleZlibBPP
#endif
#ifdef ReceiveZlibBPP
#undef ReceiveZlibBPP
#endif

#define HandleZlibBPP CONCAT2E(HandleZlib,BPP)
#define ReceiveZlibBPP CONCAT2E(ReceiveZlib,BPP)
#define flushZlibBPP CONCAT2E(flushZlib,BPP)
#define CARDBPP CONCAT2E(CARD,BPP)

Bool
VNCViewer::HandleZlibBPP (sgVNCViewer *ct, int rx, int ry, int rw, int rh)
{
    rfbZlibHeader hdr;
    int remaining;
    int inflateResult;
    ...
    if ( decompStreamInited == False ) {
        ...
        if ( inflateResult != Z_OK ) {
            aLog("inflateInit returned error: %d, msg: %s\n",
    ...
    while (( remaining > 0 ) &&
        ...
        if ( inflateResult == Z_NEED_DICT ) {
            aLog("zlib inflate needs a dictionary!\n");
            return False;
        }
        if ( inflateResult < 0 ) {
            aLog("zlib inflate returned error: %d, msg: %s\n",
                 inflateResult, decompStream.msg);
            return False;
        }
        ...
        if (( decompStream.avail_in > 0 ) &&
            ...
            aLog("zlib inflate ran out of space!\n");
            return False;
        }
    ...
    if ( inflateResult == Z_OK ) {
        ...
        ct->CopyDataToScreen(raw_buffer, rx, ry, rw, rh);
